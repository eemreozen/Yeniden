...
Aylık görev tamamlandığında `gamification-service`, `ecocoin-service`'i `reason=QUEST_COMPLETED` ve `idempotencyKey=quest:{userId}:{questId}` ile çağırır veya event yayınlar. Görev ödülleri günlük tavana dahil değildir, aylık tavana dahildir.
...
Tek bir sinyal engelleme sebebi değildir. **İki veya daha fazla sinyal** birlikte geldiğinde teslim `PENDING_REVIEW` olur ve puan moderatör onayına kadar verilmez.
...
```
Sonuç 0–100 arasına sıkıştırılır. 30'un altı risk sinyali üretir; 15'in altı ilan açmayı ve talep göndermeyi engeller.
...
- Durum sorgusu için `GET /vouchers/{idempotencyKey}` gereklidir.
- YENİDEN tarafında kullanıcı kimliği partnere gönderilmez; her partner için türetilmiş, geri çevrilemez bir `externalUserRef` kullanılır.
...
### Ödül stoğu
`Reward.stock` hold anında düşürülür, `RELEASE` durumunda geri artırılır.
