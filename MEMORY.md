...
- 2026-09-09: Backend uyumluluk taraması ve düzeltme dilimi tamamlandı. `ListingPublishedEvent` artık gamification tarafından kendi kuyruğunda tüketiliyor; `listingsPublished`, rozet ve aylık görev metriği idempotent güncelleniyor. Katalog producer-owned kuyruk tanımından arındırıldı.
- 2026-09-09: Catalog şeması Hibernate `ddl-auto=update` + `data.sql` yerine Flyway V1 baseline/migrasyonuna geçirildi; mevcut şemalar için baseline version 0 korundu. Identity OTP namespace varsayılanı eklendi; Compose identity `local` profiliyle ve dışarıdan zorunlu en az 32 baytlık `OTP_HMAC_SECRET` ile başlıyor.
- 2026-09-09: PostgreSQL Compose healthcheck'i init sırasında çalışan Unix-socket-only geçici sunucuyu hazır saydığı için tüm DB servisleri yarışla kapanıyordu. Kontrol TCP `127.0.0.1:5432` üzerinden yapılacak şekilde düzeltildi. Java 21 + Colima/Testcontainers ile tam Maven reactor testi geçti. Gateway sağlık uçları iki grupta 200 verdi; katalogdan ilan yayınlama → RabbitMQ → gamification DB akışı gerçek konteynerlerde doğrulandı.
- 2026-09-09: Yerel Colima 2 GB RAM ile yedi JVM'i aynı anda taşıyamadı; gateway/catalog/gamification `OOMKilled=true`, exit 137 oldu. Bu kod/kontrat hatası değildir. Tam Compose için README'ye en az 4 GB Docker VM RAM gereksinimi eklendi; smoke doğrulaması bellek nedeniyle iki servis grubunda yapıldı.
